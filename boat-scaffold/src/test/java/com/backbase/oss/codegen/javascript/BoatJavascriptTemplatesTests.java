 ...
 */
class BoatJavascriptTemplatesTests {
    static final String PROP_BASE = BoatJavascriptTemplatesTests.class.getSimpleName() + ".";
    static final boolean PROP_FAST = Boolean.valueOf(System.getProperty(PROP_BASE + "fast", "true"));
    static final String TEST_OUTPUT = System.getProperty(PROP_BASE + "output", "target/boat-javascript-templates-tests");
    /**
    ...
    Stream<DynamicTest> testStream(BoatJavascriptTemplatesTests.Combination param) {
        return concat(
                Stream.of(dynamicTest("generate", () -> generate(param))),
                stream(getClass().getDeclaredMethods())
                        .filter(m -> m.isAnnotationPresent(BoatJavascriptTemplatesTests.Check.class))
    ...
    public void npmName() {
        assertThat(
                findPattern(selectFiles("/package\\.json$"), "\"name\": \"@example/javascript-http\""),
    ...
    private List<File> generateFrom(String templates) {
        ...
        cf.setGeneratorName(BoatJavascriptGenerator.NAME);
        ...
        cf.addAdditionalProperty(BoatJavascriptGenerator.WITH_MOCKS, this.param.withMocks);
        if (this.param.npmRepository) {
            cf.addAdditionalProperty(BoatJavascriptGenerator.NPM_REPOSITORY, "https://registry.example.com/npm");
        }
        if (this.param.npmName) {
            cf.addAdditionalProperty(AbstractTypeScriptClientCodegen.NPM_NAME, "@example/javascript-http");
        }
        if (this.param.apiModulePrefix) {
            cf.addAdditionalProperty(BoatJavascriptGenerator.API_MODULE_PREFIX, "Boat");
        }
        if (this.param.serviceSuffix) {
            cf.addAdditionalProperty(BoatJavascriptGenerator.SERVICE_SUFFIX, "Gateway");
        }

        if (!Objects.isNull(this.param.ngVersion)) {
            cf.addAdditionalProperty(BoatJavascriptGenerator.NG_VERSION, this.param.ngVersion);
        }
        if (!Objects.isNull(this.param.foundationVersion)) {
            cf.addAdditionalProperty(BoatJavascriptGenerator.FOUNDATION_VERSION, this.param.foundationVersion);
        }

        if (!Objects.isNull(this.param.specVersion)) {
            cf.addAdditionalProperty(BoatJavascriptGenerator.SPEC_VERSION, this.param.specVersion);
        }

        if (!Objects.isNull(this.param.buildDist)) {
            cf.addAdditionalProperty(BoatJavascriptGenerator.BUILD_DIST, this.param.buildDist);
        }
        ...
        cf.addAdditionalProperty(CodegenConstants.ARTIFACT_ID, "boat-javascript-templates-tests");
